...
public abstract class JettyHttpServer implements HttpServer {
  ...
  protected static final String FILES_URL_MATCH = String.format("/%s/*", Constants.FILES_ROOT);
  protected static final String[] GZIPPABLE_METHODS =
      new String[] {"POST", "PUT", "PATCH", "DELETE"};

  protected final HttpServerFactoryOptions options;
  ...
  protected void finalizeSetup(HttpServerFactoryOptions options) {
    if (options.jettySettings().getStopTimeout().isEmpty()) {
      jettyServer.setStopTimeout(1000);
    }
  }

  protected Server createServer(HttpServerFactoryOptions options) {
