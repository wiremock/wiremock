...
import com.github.tomakehurst.wiremock.core.Constants;
...
public class Jetty11HttpServer extends JettyHttpServer {
  ...
  private ServletContextHandler addMockServiceContext(
    ...
    mockServiceContext.setAttribute(stubRequestHandler.getClass().getName(), stubRequestHandler);
    mockServiceContext.setAttribute(Notifier.KEY, notifier);
    mockServiceContext.setAttribute(
        HttpServerFactoryOptions.ChunkedEncodingPolicy.class.getName(), chunkedEncodingPolicy);
    mockServiceContext.setAttribute("browserProxyingEnabled", browserProxyingEnabled);
    ServletHolder servletHolder =
        mockServiceContext.addServlet(options.wireMockHandlerDispatchingServletClass(), "/");
    servletHolder.setInitOrder(1);
    servletHolder.setInitParameter(
        Constants.REQUEST_HANDLER_CLASS_KEY, options.stubRequestHandlerClass().getName());
    servletHolder.setInitParameter(
        FaultInjectorFactory.INJECTOR_CLASS_KEY, JettyFaultInjectorFactory.class.getName());
    servletHolder.setInitParameter(Constants.SHOULD_FORWARD_TO_FILES_CONTEXT, "true");
    ...
    if (asynchronousResponseSettings.isEnabled()) {
      ...
      mockServiceContext.setAttribute(
          options.wireMockHandlerDispatchingServletClass().getSimpleName()
              + ".asynchronousResponseExecutor",
  ...
  private ServletContextHandler addAdminContext(Object adminRequestHandler, Notifier notifier) {
    ServletContextHandler adminContext =
        new ServletContextHandler(jettyServer, Constants.ADMIN_CONTEXT_ROOT);
    ...
    ServletHolder servletHolder =
        adminContext.addServlet(options.wireMockHandlerDispatchingServletClass(), "/");
    servletHolder.setInitParameter(
        Constants.REQUEST_HANDLER_CLASS_KEY, adminRequestHandler.getClass().getName());
    adminContext.setAttribute(adminRequestHandler.getClass().getName(), adminRequestHandler);
    ...
    adminContext.addServlet(options.notMatchedServletClass(), "/not-matched");
