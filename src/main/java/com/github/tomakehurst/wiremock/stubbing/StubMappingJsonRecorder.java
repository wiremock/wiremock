...
public class StubMappingJsonRecorder implements RequestListener {
  private static final List<String> ALLOWABLE_GZIP_ACCEPT_ENCODING_HEADERS =
  ...
  private byte[] bodyDecompressedIfRequired(Response response) {
    ...
    if (ALLOWABLE_GZIP_ACCEPT_ENCODING_HEADERS.stream()
        .anyMatch(contentEncodingHeader::containsValue)) {
