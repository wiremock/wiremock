...
public class ResponseDribbleAcceptanceTest {
  ...
  /**
   * @Test public void requestIsSuccessfulButTakesLongerThanSocketTimeoutWhenDribbleIsEnabled()
   * throws Exception { doNotRunOnMacOSXInCI();
   *
   * <p>stubFor( get("/delayedDribble") .willReturn( ok().withBody(BODY_BYTES)
   * .withChunkedDribbleDelay(BODY_BYTES.length, DOUBLE_THE_SOCKET_TIMEOUT)));
   *
   * <p>long start = System.currentTimeMillis(); ClassicHttpResponse response =
   * httpClient.execute(new HttpGet(wireMockRule.url("/delayedDribble"))); byte[] responseBody =
   * response.getEntity().getContent().readAllBytes(); int duration = (int)
   * (System.currentTimeMillis() - start);
   *
   * <p>assertThat(response.getCode(), is(200)); assertThat(responseBody, is(BODY_BYTES));
   * assertThat(duration, greaterThanOrEqualTo(SOCKET_TIMEOUT_MILLISECONDS)); assertThat((double)
   * duration, isWithinTolerance(DOUBLE_THE_SOCKET_TIMEOUT, TOLERANCE)); }
   */
  /**
   * @Test public void servesAStringBodyInChunks() throws Exception { doNotRunOnMacOSXInCI();
   *
   * <p>final int TOTAL_TIME = 500;
   *
   * <p>stubFor( get("/delayedDribble") .willReturn( ok().withBody("Send this in many pieces
   * please!!!") .withChunkedDribbleDelay(2, TOTAL_TIME)));
   *
   * <p>long start = System.currentTimeMillis(); ClassicHttpResponse response =
   * httpClient.execute(new HttpGet(wireMockRule.url("/delayedDribble"))); String responseBody =
   * EntityUtils.toString(response.getEntity()); double duration = (double)
   * (System.currentTimeMillis() - start);
   *
   * <p>assertThat(response.getCode(), is(200)); assertThat(responseBody, is("Send this in many
   * pieces please!!!")); assertThat(duration, isWithinTolerance(TOTAL_TIME, TOLERANCE)); }
   */
  /**
   * @Test public void requestIsSuccessfulAndBelowSocketTimeoutWhenDribbleIsDisabled() throws
   * Exception { doNotRunOnMacOSXInCI();
   *
   * <p>stubFor(get("/nonDelayedDribble").willReturn(ok().withBody(BODY_BYTES)));
   *
   * <p>long start = System.currentTimeMillis(); ClassicHttpResponse response =
   * httpClient.execute(new HttpGet(wireMockRule.url("/nonDelayedDribble"))); byte[] responseBody =
   * response.getEntity().getContent().readAllBytes(); int duration = (int)
   * (System.currentTimeMillis() - start);
   *
   * <p>assertThat(response.getCode(), is(200)); assertThat(BODY_BYTES, is(responseBody));
